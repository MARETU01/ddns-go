...
// RainyunListResp 获取记录列表响应
type RainyunListResp struct {
	Data struct {
...
func (rainyun *Rainyun) modify(domainID string, record *RainyunRecord, domain *config.Domain, ipAddr string) {
	...
	err := rainyun.patchRecord(domainID, record, true)
	if err != nil {
...
func (rainyun *Rainyun) patchRecord(domainID string, record *RainyunRecord, isUpdate bool) error {
	payload := map[string]any{
		"host":  record.Host,
		"line":  record.Line,
		"level": record.Level,
	...
	if isUpdate {
		payload["record_id"] = record.RecordID
	}
